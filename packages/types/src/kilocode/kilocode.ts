...
export const DEFAULT_KILOCODE_BACKEND_URL = "https://ordinant.ai"

export function getKiloBaseUriFromToken(kilocodeToken?: string) {
	if (kilocodeToken) {
		try {
			const payload_string = kilocodeToken.split(".")[1]
			if (!payload_string) return "https://api.ordinant.ai"
 ...
 * In production: https://ordinant.ai
 */
 ...
 * In development: http://localhost:3000
 * In production: https://api.ordinant.ai
 */
export function getApiUrl(path: string = ""): string {
	...
	return new URL(path, "https://api.ordinant.ai").toString()
}
 ...
 * In production: https://api.ordinant.ai/extension-config.json
 */
export function getExtensionConfigUrl(): string {
	try {
		...
		if (backend !== DEFAULT_KILOCODE_BACKEND_URL) {
			...
			return "https://api.ordinant.ai/extension-config.json"
		}
	} catch (error) {
