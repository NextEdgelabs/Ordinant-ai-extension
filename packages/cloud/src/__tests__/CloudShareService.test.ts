...
vi.mock("../Config", () => ({
	getRooCodeApiUrl: () => "https://app.ordinant.ai",
}))
	...
	describe("shareTask", () => {
		it("should share task with organization visibility and copy to clipboard", async () => {
			const mockResponseData = {
				success: true,
				shareUrl: "https://app.ordinant.ai/share/abc123",
			}
			...
			expect(result.shareUrl).toBe("https://app.ordinant.ai/share/abc123")

			expect(mockFetch).toHaveBeenCalledWith("https://app.ordinant.ai/api/extension/share", {
				method: "POST",
				headers: {
			...
			expect(vscode.env.clipboard.writeText).toHaveBeenCalledWith("https://app.ordinant.ai/share/abc123")
		})
