	...
	describe("initialize", () => {
		it("should register sync keys when settings sync is enabled", async () => {
			...
			expect(mockGlobalState.setKeysForSync).toHaveBeenCalledWith([
				"ordinant-ai.allowedCommands",
				"ordinant-ai.deniedCommands",
				"ordinant-ai.autoApprovalEnabled",
				"ordinant-ai.fuzzyMatchThreshold",
				"ordinant-ai.diffEnabled",
				"ordinant-ai.directoryContextAddedContext",
				"ordinant-ai.language",
				"ordinant-ai.customModes",
				"ordinant-ai.firstInstallCompleted",
				"ordinant-ai.telemetrySetting",
			])
		...
		it("should use default value true when setting is not configured", async () => {
			...
			expect(mockGlobalState.setKeysForSync).toHaveBeenCalledWith(
				expect.arrayContaining(["ordinant-ai.allowedCommands", "ordinant-ai.deniedCommands"]),
			)
		})
		...
		it("should work without outputChannel parameter", async () => {
			...
			expect(mockGlobalState.setKeysForSync).toHaveBeenCalledWith(
				expect.arrayContaining(["ordinant-ai.allowedCommands"]),
			)
		})
	})
