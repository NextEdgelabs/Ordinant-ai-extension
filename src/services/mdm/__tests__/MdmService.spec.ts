...
vi.mock("@roo-code/cloud", () => ({
	...
	PRODUCTION_CLERK_BASE_URL: "https://clerk.ordinant.ai",
}))
	...
	t: vi.fn((key: string) => {
		const translations: Record<string, string> = {
			"mdm.errors.cloud_auth_required":
				"Your organization requires Ordinant.ai Cloud authentication. Please sign in to continue.",
			"mdm.errors.organization_mismatch":
				"You must be authenticated with your organization's Ordinant.ai Cloud account.",
...
describe("MdmService", () => {
	...
	beforeEach(() => {
		...
		mockGetClerkBaseUrl.mockReturnValue("https://dev.clerk.ordinant.ai")
	...
	describe("compliance checking", () => {
		...
		it("should be non-compliant when not authenticated", async () => {
			...
			if (!compliance.compliant) {
				expect(compliance.reason).toContain("Your organization requires Ordinant.ai Cloud authentication")
			}
		})
