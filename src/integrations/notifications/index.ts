...
async function showMacOSNotification(options: NotificationOptions): Promise<void> {
	...
	try {
		...
		// Add Ordinant.ai logo
		const extensionUri = vscode.extensions.getExtension(`ordinant-ai.ordinant-ai`)!.extensionUri
		const iconPath = vscode.Uri.joinPath(extensionUri, "assets", "icons", "ordinant.png").fsPath
...
"@
    ...
    [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("Ordinant.ai").Show($toast)
    `
...
export async function showSystemNotification(options: NotificationOptions): Promise<void> {
	try {
		const { title = "Ordinant.ai", message } = options
