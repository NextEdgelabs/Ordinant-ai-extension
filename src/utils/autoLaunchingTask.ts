...
export async function checkAndRunAutoLaunchingTask(context: vscode.ExtensionContext): Promise<void> {
	...
	try {
		...
		await vscode.commands.executeCommand("ordinant-ai.SidebarProvider.focus")

		vscode.commands.executeCommand("ordinant-ai.newTask", config) // Pass the full config to newTask
	} catch (error) {
