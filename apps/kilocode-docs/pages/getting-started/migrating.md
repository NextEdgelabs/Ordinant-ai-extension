...
description: "Guide for migrating to Ordinant.ai from other AI coding tools"
---

# Migrating from Cursor or Windsurf

Quickly migrate your custom rules from Cursor or Windsurf to Ordinant.ai. The process typically takes just a few minutes per project.

{% callout type="info" title="Two Workflow Approaches"%}

Ordinant.ai supports **two complementary workflows**—choose the one that fits your style, or use both:
...
## Why Ordinant.ai's Rules System?

Ordinant.ai simplifies AI configuration while adding powerful new capabilities:
...
### What's Different in Ordinant.ai

| Cursor                                      | Ordinant.ai                               | Key Difference                              |
| ------------------------------------------- | ----------------------------------------- | ------------------------------------------- |
...
**2. Create Ordinant.ai directory:**
...
Cursor's `globs` field specifies which files a rule applies to. Ordinant.ai handles this through **mode-specific directories** instead.
...
**Ordinant.ai approach (Option 1 - Mode-specific directory):**
...
**Ordinant.ai approach (Option 2 - Custom mode):**
...
Cursor supports nested `.cursor/rules/` directories. Ordinant.ai uses flat structure with descriptive names:

```bash
# Cursor: .cursor/rules/backend/server/api-rules.mdc
# Ordinant.ai: .kilocode/rules/backend-server-api-rules.md
```
...
| Windsurf                                                       | Ordinant.ai                    | Key Difference                              |
| -------------------------------------------------------------- | ------------------------------ | ------------------------------------------- |
...
Windsurf configures activation through the GUI. In Ordinant.ai, file organization replaces GUI configuration:

| Windsurf GUI Mode        | Ordinant.ai Equivalent                                      |
| ------------------------ | ----------------------------------------------------------- |
...
All three tools support the `AGENTS.md` standard. If you have one, it works in Ordinant.ai automatically:

```bash
# Verify it exists
ls -la AGENTS.md

# That's it - Ordinant.ai loads it automatically (enabled by default)
```

**Important:** Use uppercase `AGENTS.md` (not `agents.md`). Ordinant.ai also accepts `AGENT.md` (singular) as a fallback.

**Note:** Both `AGENTS.md` and `AGENT.md` are write-protected files in Ordinant.ai and require user approval to modify.

## Understanding Mode-Specific Rules

This is Ordinant.ai's unique feature that replaces both Cursor's `globs` and Windsurf's activation modes.
...
- [ ] **Verify rules loaded:** Click law icon (⚖️) in Ordinant.ai panel
- [ ] **Test rule application:** Ask Ordinant.ai to perform tasks following your rules
...
- **After migrating:** Organize files accordingly in Ordinant.ai

### Nested Rules Flattened

Cursor's nested directories don't map to Ordinant.ai. Flatten with descriptive names:
...
- **Check setting:** Verify "Use Agent Rules" is enabled in Ordinant.ai settings (enabled by default)
- **Reload:** Restart VS Code if needed

### Choosing Your Workflow

Ordinant.ai supports **both autocomplete and chat-driven workflows**. Choose the approach that fits your coding style, or combine them:
...
- [Join our Discord](https://ordinant.ai/discord) for migration support
