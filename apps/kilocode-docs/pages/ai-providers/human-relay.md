...
The Human Relay provider allows you to use Ordinant.ai with web-based AI models like ChatGPT or Claude without needing an API key. Instead, it relies on you to manually relay messages between Ordinant.ai and the AI's web interface.

## How it Works

1.  **Select Human Relay**: Choose "Human Relay" as your API provider in Ordinant.ai's settings. No API key is required.
2.  **Initiate a Request**: Start a chat or task with Ordinant.ai as usual.
...
6.  **Paste Back to Ordinant.ai**: Return to the dialog box in VS Code, paste the AI's response into the designated field, and click "Confirm".
7.  **Continue**: Ordinant.ai will process the response as if it came directly from an API.
