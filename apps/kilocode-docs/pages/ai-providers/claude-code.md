...
In January 2026, Anthropic decided to restrict Claude Code CLI to official Claude Code clients. Claude Code credentials cannot be used in Ordinant.ai or other third-party harnesses.

For continued use of Anthropic models in Ordinant.ai, please use the [Anthropic API provider](/ai-providers/anthropic) with an API key instead.
{% /callout %}

# Using Claude Code With Ordinant.ai

Claude Code is Anthropic's official CLI that provides direct access to Claude models from your terminal. Using Claude Code with Ordinant.ai lets you leverage your existing CLI setup without needing separate API keys.
...
If this environment variable is set on your system, the `claude` tool may use it for authentication instead of the interactive `/login` method. When Ordinant.ai executes the tool, it will accurately reflect that an API key is being used, as this is the underlying behavior of the `claude` CLI itself.
{% /callout %}
...
1. **Open Ordinant.ai Settings:** Click the gear icon ({% codicon name="gear" /%}) in the Ordinant.ai panel.
...
- **Enhance Prompt Feature:** Full compatibility with Ordinant.ai's Enhance Prompt feature, allowing you to automatically improve and refine your prompts before sending them to Claude.
