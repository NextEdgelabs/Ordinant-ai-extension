...
description: "Integrate Ordinant.ai with your shell environment"
---

# Terminal Shell Integration

Terminal Shell Integration is a key feature that enables Ordinant.ai to execute commands in your terminal and intelligently process their output. This bidirectional communication between the AI and your development environment unlocks powerful automation capabilities.

## What is Shell Integration?

Shell integration is automatically enabled in Ordinant.ai and connects directly to your terminal's command execution lifecycle without requiring any setup from you. This built-in feature allows Ordinant.ai to:
...
When Ordinant.ai needs to perform tasks like installing dependencies, starting a development server, or analyzing build errors, shell integration works behind the scenes to make these interactions smooth and effective.

## Getting Started with Shell Integration

Shell integration is built into Ordinant.ai and works automatically in most cases. If you see "Shell Integration Unavailable" messages or experience issues with command execution, try these solutions:
...
Ordinant.ai provides several settings to fine-tune shell integration. Access these in the Ordinant.ai sidebar under Settings → Terminal.
...
Adds a small pause after running commands to help Ordinant.ai capture all output correctly. This setting can significantly impact shell integration reliability due to VSCode's implementation of terminal integration across different operating systems and shell configurations:
...
Helps PowerShell run the same command multiple times in a row. Enable this if you notice Ordinant.ai can't run identical commands consecutively in PowerShell.
...
Prevents ZSH from adding special characters at the end of output lines that can confuse Ordinant.ai when reading terminal results.
...
Makes Ordinant.ai work better with the popular [Oh My Zsh](https://ohmyz.sh/) shell customization framework. Turn this on if you use Oh My Zsh and experience terminal issues.
...
Improves compatibility if you use the Powerlevel10k theme for ZSH. Turn this on if your fancy terminal prompt causes issues with Ordinant.ai.
...
Helps Ordinant.ai work with custom ZSH configurations without interfering with your personal shell settings and customizations.
...
**Alternative**: Enable the Powerlevel10k Integration setting in Ordinant.ai.
...
1. **Set Terminal Command Delay**:
    ...
    - Set the Terminal Command Delay to 50ms in Ordinant.ai settings
...
2. **Roll Back VSCode Version**:
    ...
    - No backup of Ordinant.ai settings needed
...
4. **ZSH Users**:
    - Try enabling some or all ZSH-related workarounds in Ordinant.ai settings
...
**Issue**: If text is already typed in the terminal when Ordinant.ai tries to run a command, Ordinant.ai will press Ctrl+C first to clear the line, which can interrupt running processes.

**Workaround**: Make sure your terminal prompt is empty (no partial commands typed) before asking Ordinant.ai to execute terminal commands.

### Multi-line Command Issues

**Issue**: Commands that span multiple lines can confuse Ordinant.ai and may show output from previous commands mixed in with current output.
...
1. **Premature Completion**: PowerShell sometimes tells Ordinant.ai a command is finished before all the output has been shown.
...
1. Check the [Ordinant.ai GitHub Issues](https://github.com/Ordinant-ai/ordinant-ai-extension/issues) to see if others have reported similar problems
2. If not, create a new issue with details about your operating system, VSCode/Cursor version, and the steps you've tried

For additional help, join our [Discord](https://ordinant.ai/discord).
