...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Ordinant.ai and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Ordinant.ai) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Ordinant.ai
...
- Single-client scenarios (one Ordinant.ai instance per server)
...
1. The client (Ordinant.ai) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from Ordinant.ai
...
STDIO servers run locally on the same machine as Ordinant.ai, which has several important implications:
...
- **Execution**: Starts and stops with Ordinant.ai (child process lifecycle)
...
- Start when needed by Ordinant.ai
- Not require network configuration
- Need to be installed alongside Ordinant.ai or via a package manager
...
## Configuring Transports in Ordinant.ai

For detailed information on configuring STDIO and SSE transports in Ordinant.ai, including example configurations, see the [Understanding Transport Types](/features/mcp/using-mcp-in-kilo-code#understanding-transport-types) section in the Using MCP in Ordinant.ai guide.
